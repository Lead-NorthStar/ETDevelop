...
namespace Jolt
{
    internal unsafe class NativeHandlePool
    {
        ...
        public NativeHandle<T> CreateHandle<T>(T* ptr) where T : unmanaged
        {
            ...
            //TODO:Assert.IsTrue(resource.Pointer == IntPtr.Zero); // assert any previous resource was disposed
        ...
        public T* GetPointer<T>(NativeHandle<T> handle) where T : unmanaged
        {
            ...
            if (resource.Pointer == IntPtr.Zero || handle.Version != resource.Version)
            {
